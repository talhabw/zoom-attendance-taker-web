<template>
  <v-main>
    <v-container>
      <v-row>
        <v-col cols="12">
          <v-expansion-panels>
            <v-expansion-panel
              v-for="meeting in getMeetings"
              :key="meeting.id"
              @click="meetingChanged(!meeting.end_time, meeting.id)"
            >
              <v-expansion-panel-header>
                <v-row style="line-height: 35px;" class="text-center">
                  <v-col
                    v-if="!meeting.end_time"
                    cols="1"
                    class="green--text text--darken-2 text-h6 font-weight-bold"
                  >
                    {{ "LIVE" }}
                  </v-col>
                  <v-col
                    v-else
                    cols="1"
                    class="red--text text-h6 font-weight-bold"
                  >
                    {{ "END" }}
                  </v-col>
                  <v-col cols="3" class="font-weight-medium">
                    {{ meeting.start_time }}
                  </v-col>
                  <v-col>
                    {{ meeting.topic }}
                  </v-col>
                </v-row>
              </v-expansion-panel-header>
              <v-expansion-panel-content>
                <v-row no-gutters>
                  <v-col cols="12">
                    Started at : {{ formatDate(meeting.start_time) }}
                    <br />
                    Ended at :
                    {{
                      meeting.end_time
                        ? formatDate(meeting.end_time)
                        : "Still continuing"
                    }}
                    <br />
                    Duration :
                    {{
                      meeting.duration
                        ? meeting.duration
                        : Math.floor(timeDiff(meeting.start_time, nowDate))
                    }}
                    <br />
                    <span v-if="getLoading == false">
                      Participant Count :
                      {{
                        meeting.end_time
                          ? getReportPolling.verified_members.length
                          : getParticipants.length
                      }}
                    </span>
                    <br />
                  </v-col>
                </v-row>
                <v-divider class="my-10" />
                <v-expansion-panels>
                  <v-expansion-panel @click="participantDetails()">
                    <v-expansion-panel-header>
                      <v-row class="text-center">
                        <v-col>
                          Participant Details
                        </v-col>
                      </v-row>
                    </v-expansion-panel-header>
                    <v-expansion-panel-content>
                      <v-row no-gutters>
                        <v-col cols="12">
                          <v-card>
                            <v-card-title>
                              <v-text-field
                                v-model="search"
                                append-icon="mdi-magnify"
                                label="Search"
                                hide-details
                              ></v-text-field>
                            </v-card-title>
                            <v-skeleton-loader
                              color="grey darken-2"
                              type="table-tbody"
                              v-if="loading == true"
                            ></v-skeleton-loader>
                            <v-data-table
                              v-else
                              :headers="
                                !meeting.end_time
                                  ? createLiveHeaders(getParticipants)
                                  : createEndedHeaders(getReportPolling.members)
                              "
                              :items="
                                !meeting.end_time
                                  ? getParticipants
                                  : getReportPolling.members
                              "
                              :search="search"
                              :single-expand="singleExpand"
                              :expanded.sync="expanded[meeting.id]"
                              item-key="user_number"
                              show-expand
                            >
                              <template
                                v-slot:expanded-item="{ headers, item }"
                              >
                                <td :colspan="headers.length">
                                  <span
                                    class="my-10 d-block"
                                    outlined
                                    v-for="(report, index) in item.report_time"
                                    :key="index"
                                  >
                                    Joined at : {{ report.join_time }}
                                    <br />
                                    Left at : {{ report.leave_time }}
                                    <v-divider />
                                  </span>
                                </td>
                              </template>
                            </v-data-table>
                          </v-card>
                        </v-col>
                      </v-row>
                    </v-expansion-panel-content>
                  </v-expansion-panel>
                </v-expansion-panels>
              </v-expansion-panel-content>
            </v-expansion-panel>
          </v-expansion-panels>
        </v-col>
      </v-row>
    </v-container>
  </v-main>
</template>

<script>
import { mapActions, mapGetters } from "vuex"

export default {
  name: "HelloWorld",

  data: () => ({
    loading: true,
    nowDate: new Date(new Date().toLocaleString("en")).toISOString(),
    search: "",
    singleExpand: true,
    expanded: [],
  }),
  methods: {
    ...mapActions([
      "fetchMeetings",
      "fetchParticipants",
      "fetchReportPerParticipant",
      "fetchReportMeeting",
      "fetchReportPolling",
    ]),
    createLiveHeaders(obj) {
      obj = Object.keys(obj[0])
      return [
        {
          text: "Name",
          align: "start",
          value: obj[1],
        },
        { text: "Number", value: obj[2] },
        { text: "Class", value: obj[3] },
        { text: "Join Time", value: obj[4] },
        { text: "Actions", value: "data-table-expand" },
      ]
    },
    createEndedHeaders(obj) {
      obj = Object.keys(obj[0])
      return [
        {
          text: "Name",
          align: "start",
          value: obj[1],
        },
        { text: "Number", value: obj[2] },
        { text: "Class", value: obj[3] },
        { text: "Duration", value: obj[5] },
        { text: "Here", value: obj[6] },
        { text: "Actions", value: "data-table-expand" },
      ]
    },
    findParticipant(p) {
      let participant = this.getReportPolling.members.find(
        (participant) => participant.id == p.id
      )
      return participant.report_time
    },
    log(d) {
      console.log(d)
    },
    formatDate(date) {
      return new Date(date).toLocaleString("tr-TR", {
        timeZone: "Turkey",
      })
    },
    timeDiff(start, end) {
      start = new Date(start)
      end = new Date(end)
      let diff = Math.abs((end - start) / 36e5)

      return diff * 60
    },
    meetingChanged(live, meetingId) {
      /* switch (live) {
        case true:
          this.fetchParticipants()
          break

        case false:
          this.fetchReportPolling(meetingId)
          break

        default:
          break
      } */
      this.fetchReportPolling(meetingId)
    },
    participantDetails() {
      this.loading = false
    },
  },
  computed: {
    ...mapGetters([
      "getLoading",
      "getMeetings",
      "getParticipants",
      "getReportMeeting",
      "getReportsPerParticipant",
      "getReportPolling",
    ]),
  },
  created() {
    this.fetchMeetings()
    this.fetchParticipants()
  },
}
</script>
